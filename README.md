...
ゲームのルールはonenight_jinro.pyに依存します。現在の設定はプレイヤー人数4〜7、カードの枚数は4人のときに村人2・占い師1・怪盗1・人狼2でプレイヤー人数が増えるごとに村人が増えます。6人以上で村人1が狂人1に置き換わります。
...
**数字を含むコマンド(`!u0`など)を送る際は常に半角英数字3文字を送ってください。**
 ...
 1. ids.txtの1行目をBotのアクセストークンに、2行目をワンナイト人狼のゲームを行うメインのテキストチャンネルのIDに書き換えてください。
 2. ```$ python onjbot.py```でbotを起動します。
 3. Discord内の好きなテキストチャンネルで```!人狼スタート```とチャットに送ることでゲームが待機状態になります。以降のコマンドはこのときのチャンネルのみ使用することを推奨します。
<<<<<<< HEAD
 4. ```!参加```をそれぞれのプレイヤーが送ることで参加が受理され、プレイヤー人数が更新されていきます。またすでに参加しているプレイヤーがもう一度送ることで参加を取り消すこともできます。`!観戦`で観戦することもできます。観戦するとゲーム開始時にDMにてすべてのプレイヤーの役職がわかります(怪盗の結果はわかりません)。観戦状態と参加状態は二者択一です。
=======
 4. ```!参加```をそれぞれのプレイヤーが送ることで参加が受理され、プレイヤー人数が更新されていきます。またすでに参加しているプレイヤーがもう一度送ることで参加を取り消すこともできます。`!観戦`を送ることで最初に配られた役職をすべて閲覧することが可能です(怪盗の結果はわかりません)。観戦状態と参加状態は二者択一です。
>>>>>>> 7c7d903... Rewrite README
...
>@PLAYER3 占う相手を選んでください。以下のコマンドをコピーして送ってください：
...
>@PLAYER1 役職を盗む相手を選んでください。以下のコマンドをコピーして送ってください：
